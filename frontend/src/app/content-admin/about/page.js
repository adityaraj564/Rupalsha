...
export default function ContentAdminAboutPage() {
  ...
  const fetchAbout = async () => {
    try {
      const data = await subAdminAPI.getAbout();
      setAbout(data.about);
      setForm({
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const data = await subAdminAPI.updateAbout(form);
      setAbout(data.about);
  ...
  const handleCoverUpload = async (e) => {
    ...
    try {
      const data = await subAdminAPI.uploadAboutCover(formData);
      setAbout(data.about);
  ...
  const handleTeamImageUpload = async (index, e) => {
    ...
    formData.append('image', file);
    try {
      const data = await subAdminAPI.uploadTeamImage(index, formData);
      setAbout(data.about);
  ...
  const handleAddMember = async () => {
    try {
      const data = await subAdminAPI.addTeamMember({
        name: 'New Member',
        role: 'Team Member',
        title: 'Team Member',
        bio: '',
  ...
  const handleRemoveMember = async (index) => {
    ...
    try {
      const data = await subAdminAPI.removeTeamMember(index);
      setAbout(data.about);
