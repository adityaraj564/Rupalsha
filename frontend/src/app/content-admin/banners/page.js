...
export default function ContentAdminBannersPage() {
  ...
  const fetchBanners = async () => {
    try {
      const data = await subAdminAPI.getBanners();
      setBanners(data);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await subAdminAPI.deleteBanner(id);
      await fetchBanners();
  ...
  const handleToggleActive = async (banner) => {
    try {
      await subAdminAPI.updateBanner(banner._id, { ...banner, isActive: !banner.isActive });
      await fetchBanners();
  ...
  const saveEdit = async (id) => {
    try {
      await subAdminAPI.updateBanner(id, { title: editForm.title, link: editForm.link });
      setEditingId(null);
      await fetchBanners();
  ...
  const handleMove = async (index, direction) => {
    ...
    try {
      await subAdminAPI.reorderBanners(newBanners.map((b) => b._id));
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-brand-charcoal dark:text-gray-100 mb-6">Banner Management</h1>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <input type="text" value={newLink} onChange={(e) => setNewLink(e.target.value)} placeholder="/products or https://..." className="w-full px-3 py-2 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-sm" />
          </div>
        </div>
        <button onClick={handleUpload} disabled={uploading} className="bg-brand-gold text-white px-6 py-2 rounded-lg hover:bg-brand-gold/90 transition-colors flex items-center gap-2 text-sm disabled:opacity-50">
          {uploading ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> Uploading...</> : <><FiUpload size={16} /> Upload Banner</>}
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
        <div className="p-4 border-b border-gray-100 dark:border-gray-700">
          <h2 className="font-semibold text-brand-charcoal dark:text-gray-100">
            Banners ({banners.filter((b) => b.isActive).length} active / {banners.length} total)
          </h2>
        </div>
        {banners.length === 0 ? (
                ...
                <div className="flex-1 min-w-0">
                  {editingId === banner._id ? (
                    <div className="space-y-1.5">
                      <input type="text" value={editForm.title} onChange={(e) => setEditForm({ ...editForm, title: e.target.value })} placeholder="Banner title" className="w-full px-2 py-1 text-sm border border-gray-200 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100" autoFocus />
                      <input type="text" value={editForm.link} onChange={(e) => setEditForm({ ...editForm, link: e.target.value })} placeholder="Link" className="w-full px-2 py-1 text-xs border border-gray-200 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100" />
                      <div className="flex gap-1">
                        <button onClick={() => saveEdit(banner._id)} className="p-1 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/30 rounded"><FiCheck size={14} /></button>
                        <button onClick={() => setEditingId(null)} className="p-1 text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 rounded"><FiX size={14} /></button>
                      </div>
                    </div>
                  ) : (
                    <div className="cursor-pointer" onClick={() => startEdit(banner)}>
                      <p className="font-medium text-brand-charcoal dark:text-gray-100 truncate flex items-center gap-1.5">{banner.title || 'Untitled Banner'} <FiEdit2 size={12} className="text-gray-400" /></p>
                      {banner.link && <p className="text-xs text-gray-400 truncate">{banner.link}</p>}
                    </div>
                  )}
                </div>
                <button onClick={() => handleToggleActive(banner)} className={`flex items-center gap-1 text-sm px-3 py-1 rounded-full ${banner.isActive ? 'bg-green-50 text-green-600 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-400 dark:bg-gray-700 dark:text-gray-500'}`}>
                  {banner.isActive ? <FiToggleRight size={16} /> : <FiToggleLeft size={16} />}
                  {banner.isActive ? 'Active' : 'Hidden'}
                </button>
                <button onClick={() => handleDelete(banner._id)} className="p-2 text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg"><FiTrash2 size={16} /></button>
              </div>
            ))}
          </div>
        )}
