...
export default function ContentAdminBlogsPage() {
  ...
  const fetchBlogs = async () => {
    ...
    try {
      ...
      const data = await subAdminAPI.getBlogs(params);
      setBlogs(data.blogs || []);
    } catch (err) {
  ...
  const openEdit = async (blog) => {
    try {
      const data = await subAdminAPI.getBlog(blog._id);
      const b = data.blog;
      setEditingBlog(b);
      setForm({
        title: b.title || '',
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (editingBlog) {
        await subAdminAPI.updateBlog(editingBlog._id, formData);
        toast.success('Blog updated');
      } else {
        await subAdminAPI.createBlog(formData);
  ...
  const handleToggle = async (blog) => {
    try {
      const data = await subAdminAPI.toggleBlogStatus(blog._id);
  ...
  const handleDelete = async (blog) => {
    ...
    try {
      await subAdminAPI.deleteBlog(blog._id);
